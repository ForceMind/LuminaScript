...
### AI 用量归属与日志兼容
- 项目相关 AI（设定、摘要、大纲、正文、场次重写及提示词转换）按项目所有者检查和累计额度；独立 `/content/review` 按当前操作者计费。
- AI 日志的 `user_id` / `user_name` 始终表示实际操作者；新增 `billed_user_id` / `billed_username` 表示计费对象，二者不能混用。
- 历史日志的新计费字段保持 `null`，仍按历史 `user_id` 统计，不回填或追溯改写；UI 将其标记为“历史记账”。
- 原 `user_id` 日志筛选仍是操作者筛选。备份与导入按用户 ID 映射计费对象，不把展示用户名当作计费依据。
- 日/月额度使用 UTC 周期；新 AI 日志按同一 UTC 口径写入，历史时间戳不重写。
- 额度检查仅阻止新的模型调用，包括逻辑补齐和底层重试/备用配置调用；缓存读取、静态选项、工作稿保存/恢复及完成查询不因额度耗尽而被阻断。
- 本系统采用调用前检查的软额度：已经发出的并发请求仍可能使累计量超过阈值，不提供预付冻结或硬上限保证。
- 队列记录实际发起/重试操作者，Worker 按项目所有者计费；旧任务没有操作者元数据时兼容回退到项目所有者。
